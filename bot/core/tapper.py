...
class Tapper:
    ...
    async def get_strict_data(self, http_client: aiohttp.ClientSession):
        try:
            ...
            response = await http_client.post('https://bot-api.bool.network/bool-tg-interface/user/user/strict',
                                              json=json_data)
    ...
    async def get_staking_record(self, http_client: aiohttp.ClientSession, page: int) -> dict:
        try:
            response = await http_client.get(f'https://bot-api.bool.network/bool-tg-interface/user/vote:devices?'
            ...
            for record in records:
                if record['voterCount'] < 190 and record['deviceState'] == 'SERVING':
                    return record
    ...
    async def processing_tasks(self, http_client: aiohttp.ClientSession):
        try:
            ...
            response = await http_client.post('https://bot-api.bool.network/bool-tg-interface/assignment/list',
                                              json=json_data)
            response.raise_for_status()
            response_json = await response.json()
